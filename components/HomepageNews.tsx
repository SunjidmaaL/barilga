...
// EmptyCard component defined outside the function
const EmptyCard = () => (
  <div className="rounded-xl bg-gray-50 border-2 border-dashed border-gray-300 p-4 sm:p-6 md:p-8 text-center">
    <div className="mx-auto mb-3 sm:mb-4 w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-indigo-100 rounded-full flex items-center justify-center">
      <svg className="w-6 h-6 sm:w-7 md:w-8 md:h-8 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default async function HomepageNews() {
  try {
    ...
    const latestNews = (news && Array.isArray(news) && news.length > 0) ? news.slice(0, 3) : []
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
      ...
      {latestNews.length < 3 && [...Array(3 - latestNews.length)].map((_, i) => (
        <EmptyCard key={`empty-${i}`} />
      ))}
    </div>
  )
  } catch (error) {
    // If there's an error, show empty cards
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
        {[...Array(3)].map((_, i) => <EmptyCard key={i} />)}
      </div>

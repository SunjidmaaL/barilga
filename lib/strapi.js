...
export async function getNewsById(id) {
  try {
    const url = `${API_URL}/api/news2/${id}?populate=*`;
    
    if (process.env.NODE_ENV === 'development') {
      console.log('Fetching news item from:', url);
    }
    ...
    if (!res.ok) {
      ...
      console.error(`Failed to fetch news item: ${res.status} ${res.statusText}`);
      ...
      if (res.status === 404) {
        if (process.env.NODE_ENV === 'development') {
          console.warn(`News item with id ${id} not found`);
        }
        return null;
      }
      
      throw new Error(`Failed to fetch news item: ${res.status} ${res.statusText}`);
    }
    ...
    if (!data || (!data.data && !data.attributes)) {
      console.warn('No news item data received from API. Response structure:', data);
      return null;
    }
...
export async function getContacts() {
  try {
    // Use revalidate for both development and production to reduce API calls
    // Development mode can still use longer cache to reduce requests
    const cacheOption = { next: { revalidate: 300 } }; // Revalidate every 5 minutes
...
export async function getForeignRelationsImages() {
  try {
    // Try without sort first, then with createdAt sort if that doesn't work
    let res = await fetch(`${API_URL}/api/foreign-relations?populate=*`, {
      next: { revalidate: 300 } // Revalidate every 5 minutes
    });
    
    // If that fails, try with createdAt sort
    if (!res.ok && res.status === 400) {
      res = await fetch(`${API_URL}/api/foreign-relations?populate=*&sort=createdAt:desc`, {
        next: { revalidate: 300 }
      });
    }
    ...
    if (!res.ok) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.warn(`Cannot fetch foreign relations: ${res.status} ${res.statusText}. Please check if foreign-relations content type exists in Strapi and has proper permissions.`);
      }
      return [];
    }
    ...
    if (!data || !data.data) {
      if (process.env.NODE_ENV === 'development') {
        console.warn('No foreign relations data received from API. Response:', data);
      }
      return [];
    }
    
    if (process.env.NODE_ENV === 'development') {
      console.log(`✓ Successfully fetched foreign relations from Strapi. Count: ${data.data.length}`);
      console.log('Sample item structure:', data.data[0] ? JSON.stringify(data.data[0], null, 2) : 'No items');
    }
    
    return data.data;
  } catch (error) {
    if (process.env.NODE_ENV === 'development') {
      console.warn('Error fetching foreign relations images:', error.message);
    }
    return [];
  }
}

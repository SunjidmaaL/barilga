const API_URL = process.env.NEXT_PUBLIC_STRAPI_URL
  || (process.env.NODE_ENV === 'development' ? 'http://localhost:1337' : 'https://effortless-luck-023aebe70f.strapiapp.com');
...
const CACHE_SETTINGS = {
  ...
  // Reduced to 5 minutes for fresh content (was 24 hours)
  FREQUENT: process.env.NODE_ENV === 'development' ? 0 : 300, // 5 minutes (was 86400)
  // Moderately updated content (laws, licenses, foreign relations)
  // Reduced to 1 hour for better freshness (was 48 hours)
  MODERATE: process.env.NODE_ENV === 'development' ? 0 : 3600, // 1 hour (was 172800)
  // Rarely updated content (contacts, expert teams, slides)
  // Reduced to 24 hours (was 7 days)
  RARE: process.env.NODE_ENV === 'development' ? 0 : 86400, // 24 hours (was 604800)
};
...
// Member News API functions (for "Гишүүн байгууллагууд" section)
export async function getMemberNews() {
  try {
    ...
    // Try multiple API endpoint names (in case content type was named differently)
    const apiEndpoints = [
      // Strapi often pluralizes collection endpoints (e.g. member-news1 -> member-news1s)
      `${API_URL}/api/member-newss?populate=*&sort=publishedAt:desc`,
      `${API_URL}/api/member-newss?populate=*`,
      `${API_URL}/api/member-newss`,
      `${API_URL}/api/member-news?populate=*&sort=publishedAt:desc`,
      `${API_URL}/api/member-news?populate=*`,
      `${API_URL}/api/member-news`,
    ];

    // Debug logging
    if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
      console.log('[Strapi] Fetching member news from:', API_URL);
      console.log('[Strapi] Trying endpoints:', apiEndpoints);
    }
    ...
    for (const url of apiEndpoints) {
      try {
        ...
        if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
          console.log(`[Strapi] Member news response for ${url}:`, res.status, res.statusText);
        }
        ...
        if (res.ok) {
          try {
            ...
            // Debug logging
            if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
              console.log('[Strapi] Member news data structure:', {
                hasData: !!data,
            ...
            if (data && data.data !== undefined) {
              ...
              if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
                console.log(`[Strapi] Member news: Successfully fetched ${data.data?.length || 0} items`);
              }
            ...
            if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
              console.warn(`[Strapi] Member news parse failed for ${url}:`, parseError);
            }
            continue;
          }
        } else {
          ...
          if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
            if (res.status === 404) {
              console.warn(`[Strapi] Member news endpoint not found: ${url}`);
              console.warn(`[Strapi] Make sure content type 'member-news' exists in Strapi`);
            } else if (res.status === 403) {
              console.error(`[Strapi] Member news permission denied: ${url}`);
              console.error(`[Strapi] Check Public role permissions for 'member-news'`);
            } else {
              console.warn(`[Strapi] Member news error ${res.status}: ${url}`);
            }
          }
        }
      } catch (fetchError) {
        ...
        if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
          console.warn(`[Strapi] Member news fetch error for ${url}:`, fetchError);
        }
        continue;
      }
    }
    ...
    // Final check
    if (!res || !res.ok || !data || !data.data) {
      const errorKey = 'member-news:error';
      const lastErrorTime = errorCache.get(errorKey);
      const now = Date.now();
      
      if (!lastErrorTime || (now - lastErrorTime) > ERROR_CACHE_TTL) {
        if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
          console.error('[Strapi] Member news: All endpoints failed');
          console.error('[Strapi] Last error:', lastError);
          console.error('[Strapi] Last response status:', res?.status);
          console.error('[Strapi] Check that content type "member-news" exists in Strapi');
          console.error('[Strapi] Check Public role permissions for "member-news"');
          console.error('[Strapi] API URL:', API_URL);
        }
        errorCache.set(errorKey, now);
      }
      return [];
    ...
    if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
      console.log(`[Strapi] Member news: Returning ${result.length} items`);
    }

    return result;
  } catch (error) {
    if (process.env.NODE_ENV === 'development' || process.env.DEBUG_STRAPI === 'true') {
      console.error('[Strapi] Error fetching member news:', error);
      if (error instanceof Error) {
        console.error('[Strapi] Error message:', error.message);
...
export async function getMemberNewsById(id) {
  try {
    let data = null;
    let res = null;
    ...
    const apiEndpoints = [
      // Strapi often pluralizes collection endpoints (e.g. member-news1 -> member-news1s)
      `${API_URL}/api/member-newss/${id}?populate=*`,
      `${API_URL}/api/member-newss/${id}`,
      `${API_URL}/api/member-news/${id}?populate=*`,
      `${API_URL}/api/member-news/${id}`,
    ];
    ...
    for (const url of apiEndpoints) {
      ...
      if (res.ok) {
        try {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.warn(`[Strapi] Member news ${id} parse failed for ${url}, trying next`);
          }
          continue;
        }
      } else {
        ...
        if (process.env.NODE_ENV === 'development' && res.status === 404) {
          console.warn(`[Strapi] Member news item ${id} not found: ${url}`);
        }
        // Continue to next endpoint or return null if last attempt
        if (url === apiEndpoints[apiEndpoints.length - 1]) {
          return null;
        }
      }
    }
    ...
    // Support both collection response and direct item response
    if (data.data) {
      return data.data;
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error(`[Strapi] Error fetching member news item ${id}:`, error);
    }
    return null;
  }
}

...
// Fetch timeout in milliseconds (10 seconds for slower Strapi servers)
...
const CACHE_SETTINGS = {
  ...
  FREQUENT: process.env.NODE_ENV === 'development' ? 0 : 7200, // 2 hours
  // Moderately updated content (laws, licenses, foreign relations)
  MODERATE: process.env.NODE_ENV === 'development' ? 0 : 14400, // 4 hours
  // Rarely updated content (contacts, expert teams, slides)
  RARE: process.env.NODE_ENV === 'development' ? 0 : 86400, // 24 hours
};
        ...
        if (allNews && Array.isArray(allNews)) {
          ...
          if (foundNews) {
            ...
            if (foundNews.documentId && foundNews.documentId.toString() !== id.toString()) {
              ...
              const resByDocId = await fetchWithTimeout(url, {
                  next: { revalidate: CACHE_SETTINGS.FREQUENT },
        headers: getHeaders()
      });
...
export async function getTrainingAnkets() {
  try {
    ...
    if (!data || !data.data || !data.data.length || !(data.data[0].attributes?.file || data.data[0].file)) {
      ...
      let simpleRes = await fetchWithTimeout(`${API_URL}/api/training-ankets`, {
        next: { revalidate: CACHE_SETTINGS.MODERATE }
      });
...
export async function getContactsHrsById(id) {
  try {
    ...
    let res = await fetchWithTimeout(`${API_URL}/api/contact-hrs/${id}`, {
      next: { revalidate: CACHE_SETTINGS.RARE }
    });
    
    // Only retry on 500 errors, not on 400/404
    if (!res.ok && res.status === 500) {
      res = await fetchWithTimeout(`${API_URL}/api/contact-hrs/${id}?fields[0]=name&fields[1]=phone&fields[2]=order`, {
        next: { revalidate: CACHE_SETTINGS.RARE }
      });
    }
...
export async function submitTrainingRegistration(formData) {
  try {
    const res = await fetchWithTimeout(`${API_URL}/api/training-registrations`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({
        data: {
...
export async function getExpertTeams() {
  try {
    ...
    let res = await fetchWithTimeout(`${API_URL}/api/expert-teams?populate[image]=*&sort[0]=order:asc`, {
      next: { revalidate: CACHE_SETTINGS.RARE }
    });
    
    // Only retry on 500 errors, not on 400/404
    if (!res.ok && res.status === 500) {
      res = await fetchWithTimeout(`${API_URL}/api/expert-teams?sort[0]=order:asc`, {
        next: { revalidate: CACHE_SETTINGS.RARE }
      });
    }
...
export async function getExpertTeamImages() {
  try {
    // Try with specific image populate first
    let res = await fetchWithTimeout(`${API_URL}/api/expert-team-images?populate[image]=*&sort[0]=order:asc`, {
      next: { revalidate: CACHE_SETTINGS.RARE }
    });
    
    // Only retry on 500 errors, not on 400/404
    if (!res.ok && res.status === 500) {
      res = await fetchWithTimeout(`${API_URL}/api/expert-team-images?sort[0]=order:asc`, {
        next: { revalidate: CACHE_SETTINGS.RARE }
      });
    }

...
export async function getMemberNews() {
  try {
    ...
    // Populate with sort by published date
    let url = `${API_URL}/api/member-news?populate=*&sort=publishedAt:desc`;
    res = await fetchWithRetry(url, {
      next: { revalidate: CACHE_SETTINGS.FREQUENT }
    });
    ...
    if (res.ok) {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Strapi] Member news populate parse failed, trying fallback');
        }
      }
    }
    ...
    if (!res.ok || !data || !data.data) {
      ...
      if (res.ok) {
        try {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.warn('[Strapi] Member news simple query parse failed');
          }
        }
      }
    }
...
export async function getMemberNewsById(id) {
  try {
    ...
    // Populate first
    let url = `${API_URL}/api/member-news/${id}?populate=*`;
    res = await fetchWithRetry(url, {
      next: { revalidate: CACHE_SETTINGS.FREQUENT },
      headers: getHeaders()
    });
    ...
    if (res.ok) {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn(`[Strapi] Member news ${id} populate parse failed, trying fallback`);
        }
      }
    }
    ...
    if (!res.ok || !data || !data.data) {
      url = `${API_URL}/api/member-news/${id}`;
      res = await fetchWithRetry(url, {
        next: { revalidate: CACHE_SETTINGS.FREQUENT },
        headers: getHeaders()
      });
      ...
      if (res.ok) {
        try {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.warn(`[Strapi] Member news ${id} simple query parse failed`);
          }
        }
      } else {
        return null;
